...
class RWDP_Dealer_Map_Widget extends \Elementor\Widget_Base {
	...
	protected function register_controls() {
		...
		$this->add_control( 'show_print_button', [
			'label'        => __( 'Show Print/PDF Button', 'rw-dealer-portal' ),
			'type'         => \Elementor\Controls_Manager::SWITCHER,
			'label_on'     => __( 'Show', 'rw-dealer-portal' ),
			'label_off'    => __( 'Hide', 'rw-dealer-portal' ),
			'return_value' => 'yes',
		...
		$this->add_control( 'print_button_text', [
			'label'     => __( 'Print Button Text', 'rw-dealer-portal' ),
			'type'      => \Elementor\Controls_Manager::TEXT,
			'default'   => __( 'Download Contractor List', 'rw-dealer-portal' ),
		...
		// ── Style: Map — Print Button ───────────────────────────────────
		$this->start_controls_section( 'style_map_print_btn', [
			'label' => __( 'Map — Print Button', 'rw-dealer-portal' ),
			'tab'   => \Elementor\Controls_Manager::TAB_STYLE,
		] );

		$this->add_group_control( \Elementor\Group_Control_Typography::get_type(), [
			'name'     => 'map_print_btn_typography',
			'selector' => '{{WRAPPER}} .rwdp-map-print-btn',
		] );

		$this->start_controls_tabs( 'map_print_btn_tabs' );

		$this->start_controls_tab( 'map_print_btn_normal', [
			'label' => __( 'Normal', 'rw-dealer-portal' ),
		] );

		$this->add_control( 'map_print_btn_color', [
			'label'     => __( 'Text Color', 'rw-dealer-portal' ),
			'type'      => \Elementor\Controls_Manager::COLOR,
			'selectors' => [ '{{WRAPPER}} .rwdp-map-print-btn' => 'color: {{VALUE}};' ],
		] );

		$this->add_control( 'map_print_btn_bg', [
			'label'     => __( 'Background Color', 'rw-dealer-portal' ),
			'type'      => \Elementor\Controls_Manager::COLOR,
			'selectors' => [ '{{WRAPPER}} .rwdp-map-print-btn' => 'background-color: {{VALUE}};' ],
		] );

		$this->end_controls_tab();

		$this->start_controls_tab( 'map_print_btn_hover', [
			'label' => __( 'Hover', 'rw-dealer-portal' ),
		] );

		$this->add_control( 'map_print_btn_color_hover', [
			'label'     => __( 'Text Color', 'rw-dealer-portal' ),
			'type'      => \Elementor\Controls_Manager::COLOR,
			'selectors' => [ '{{WRAPPER}} .rwdp-map-print-btn:hover' => 'color: {{VALUE}};' ],
		] );

		$this->add_control( 'map_print_btn_bg_hover', [
			'label'     => __( 'Background Color', 'rw-dealer-portal' ),
			'type'      => \Elementor\Controls_Manager::COLOR,
			'selectors' => [ '{{WRAPPER}} .rwdp-map-print-btn:hover' => 'background-color: {{VALUE}};' ],
		] );
		...
		$this->add_group_control( \Elementor\Group_Control_Border::get_type(), [
			'name'      => 'map_print_btn_border',
			'selector'  => '{{WRAPPER}} .rwdp-map-print-btn',
			'separator' => 'before',
		] );

		$this->add_control( 'map_print_btn_border_radius', [
			'label'      => __( 'Border Radius', 'rw-dealer-portal' ),
			'type'       => \Elementor\Controls_Manager::DIMENSIONS,
			'size_units' => [ 'px', '%', 'em' ],
			'selectors'  => [ '{{WRAPPER}} .rwdp-map-print-btn' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};' ],
		] );

		$this->add_control( 'map_print_btn_padding', [
			'label'      => __( 'Padding', 'rw-dealer-portal' ),
			'type'       => \Elementor\Controls_Manager::DIMENSIONS,
			'size_units' => [ 'px', 'em' ],
			'selectors'  => [ '{{WRAPPER}} .rwdp-map-print-btn' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};' ],
		] );

		$this->add_control( 'map_print_btn_alignment', [
			'label'     => __( 'Alignment', 'rw-dealer-portal' ),
			'type'      => \Elementor\Controls_Manager::CHOOSE,
			'options'   => [
				'left'   => [ 'title' => __( 'Left', 'rw-dealer-portal' ), 'icon' => 'eicon-text-align-left' ],
				'center' => [ 'title' => __( 'Center', 'rw-dealer-portal' ), 'icon' => 'eicon-text-align-center' ],
				'right'  => [ 'title' => __( 'Right', 'rw-dealer-portal' ), 'icon' => 'eicon-text-align-right' ],
			],
			'default'   => 'left',
			'toggle'    => false,
			'selectors' => [ '{{WRAPPER}} .rwdp-map-print-wrap' => 'text-align: {{VALUE}};' ],
		] );
	...
	protected function render() {
		...
		$show_print_button = ( $s['show_print_button'] ?? '' ) === 'yes';
		$print_button_text = $s['print_button_text'] ?? __( 'Download Contractor List', 'rw-dealer-portal' );
		// Compatibility hook: add-on plugin defines rwdp_get_contractor_print_url().
		$print_base_url = function_exists( 'rwdp_get_contractor_print_url' ) ? rwdp_get_contractor_print_url() : home_url( '/' );
		...
		<div class="rwdp-finder__map-wrap">
			...
			<?php if ( $show_print_button ) : ?>
				<p class="rwdp-map-print-wrap" style="margin-top:12px;">
					<a href="<?php echo esc_url( $print_base_url ); ?>" class="rwdp-btn rwdp-btn--primary rwdp-map-print-btn" id="rwdp-print-results-btn" data-print-base="<?php echo esc_url( $print_base_url ); ?>" target="_blank" rel="noopener noreferrer">
						<?php echo esc_html( $print_button_text ); ?>
					</a>
				</p>
			<?php endif; ?>
		</div>
